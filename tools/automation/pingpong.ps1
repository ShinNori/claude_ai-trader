param(
    ...
    [ValidateSet('auto','dev')][string]$Channel = 'auto',
    [switch]$DryRun, [switch]$NoGit
)
$ErrorActionPreference = 'Stop'
. (Join-Path $PSScriptRoot 'handoff_common.ps1')
$engineArgs = @('pingpong','--channel',$Channel,'--agent',$StartWith,'--rounds',"$MaxRounds",'--interval',"$IntervalSec",'--stable',"$StableSec",'--max-runs',"$MaxRunsPerDay",'--timeout',"$($TurnTimeoutMin*60)")
